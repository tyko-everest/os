...
elf_ec_t parse_elf(const char *path, elf64_header_t *elf_header,
    ...
    fat32_readfile(path, 0, sizeof(elf64_header_t), elf_header);
    ...
    fat32_readfile(path, elf_header->e_phoff, elf_header->e_phentsize * MIN(elf_header->e_phnum, 2), prog_header);
    return ELF_EC_OK;
}