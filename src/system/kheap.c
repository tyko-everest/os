...
void __kernel_heap(void);
// TODO: not a fixed size heap for kernel
size_t kernel_heap_size = KHEAP_SIZE;
...
void kheap_init() {
    ...
    ((kernel_heap_segment_meta_t*) __kernel_heap)->size =
            kernel_heap_size - sizeof(kernel_heap_segment_meta_t);
    ((kernel_heap_segment_meta_t*) __kernel_heap)->free = true;
    ((kernel_heap_segment_meta_t*) __kernel_heap)->next = NULL;

    // setup linked list to start pointing to this segment
    kernel_heap_first_segment = (kernel_heap_segment_meta_t*) __kernel_heap;
}
...
void *kmalloc(size_t bytes) {
    ...
    // this will ensure alignment of all data types
    if (bytes % sizeof(uint64_t) != 0) {
        bytes = bytes / sizeof(uint64_t) * sizeof(uint64_t) + sizeof(uint64_t);
    }
    ...
    while (curr_seg != NULL) {
        ...
        // we will just take the portion of this block we need
            ...
            kernel_heap_segment_meta_t *new_block =
                    (kernel_heap_segment_meta_t*) ((size_t) curr_seg
                    + total_new_size);
